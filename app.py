...
class Garage:
    ...
    def assicura_struttura_veicolo(v):
        campi_default = {
            ...
            "km_attuali": 0, "ultimo_cambio_olio": 0, "tipo_olio_corrente": "Non specificato",
            "ultimo_filtro_olio": 0, "ultimo_filtro_aria": 0, "ultimo_filtro_abitacolo": 0, "ultimo_filtro_carburante": 0,
            "pastiglie_anteriori": 0, "pastiglie_posteriori": 0, "dischi_anteriori": 0, "dischi_posteriore": 0,
            "km_ultima_inversione": 0, "data_ultima_inversione": "Mai fatta", "data_cambio_gomme": "Non inserita",
            "scadenza_revisione": "Non inserita", "scadenza_bollo": "Non inserita", "scadenza_assicurazione": "Non inserita",
            "scadenza_tergicristalli_ant": "Non inserita", "scadenza_tergicristalli_post": "Non inserita",
            "note_storiche": {}, "storico_interventi": []
        }
...
st.set_page_config(page_title="Garage Manager Pro 📱", page_icon="🚗", layout="wide")
...
with st.sidebar.expander("➕ AGGIUNGI MEZZO"):
    ...
    if st.button("Salva Nuovo Veicolo"):
        if nuova_targa in dati:
            ...
            dati[nuova_targa] = {
                "nome_modello": nuovo_modello if nuovo_modello else "NON SPECIFICATO",
                "km_attuali": 0, "ultimo_cambio_olio": 0, "tipo_olio_corrente": "Non specificato",
                "ultimo_filtro_olio": 0, "ultimo_filtro_aria": 0, "ultimo_filtro_abitacolo": 0, "ultimo_filtro_carburante": 0,
                "pastiglie_anteriori": 0, "pastiglie_posteriori": 0, "dischi_anteriori": 0, "dischi_posteriore": 0,
                "km_ultima_inversione": 0, "data_ultima_inversione": "Mai fatta", "data_cambio_gomme": "Non inserita",
...
if targa_selezionata and targa_selezionata != "-- Seleziona --":
    ...
    modello_str = f" - {v['nome_modello'].upper()}" if v['nome_modello'].upper() != "NON SPECIFICATO" else ""
    st.header(f"🚗 Scheda Veicolo: {targa_selezionata.upper()}{modello_str}")
    
    with st.expander("✏️ Modifica Targa / Nome Modello"):
        col_m1, col_m2 = st.columns([3, 1])
        mod_nome = col_m1.text_input("Nome Modello / Descrizione", value=v.get("nome_modello", "").upper()).strip().upper()
    ...
    nuovi_km = c_km1.number_input("📊 Chilometri Attuali Mezzo", value=km, step=100)
    ...
    with col_doc:
        ...
        for nome_doc, campo in doc_list:
            ...
            dt_input = c_date.date_input(f"Seleziona {nome_doc}", value=val_default, key=f"dt_{campo}", label_visibility="collapsed")
    ...
    with col_mec:
        ...
        st.write(f"{color_o} **Olio Motore** ({spec_olio}): Usati {km_olio}/15.000 Km")

        filtri = [("Filtro Olio", "ultimo_filtro_olio"), ("Filtro Aria", "ultimo_filtro_aria"), 
                  ("Filtro Abitacolo", "ultimo_filtro_abitacolo"), ("Filtro Carburante", "ultimo_filtro_carburante")]
        for n, c in filtri:
            km_f = km - v.get(c, 0)
        ...
        st.write(f"🛑 **Pastiglie**: ANT {p_ant}/40.000 Km | POST {p_post}/60.000 Km")
        ...
        st.write(f"🔄 **Inversione Gomme**: Fatti {km_inv}/10.000 Km (Data: {data_inv})")
        st.write(f"🛞 **Ultimo Cambio Gomme**: {v.get('data_cambio_gomme', 'Non inserita')}")
        ...
        with st.expander("🔄 Inversione Pneumatici"):
            ...
            if st.button("Registra Inversione"):
                ...
                v["storico_interventi"].append({
                    "data": oggi_str, "lavoro": f"INVERSIONE GOMME [NOTE: {note_inv}]", "km": km, "costo": costo_inv
                })
                Garage.salva(dati)
    ...
    with col_rap2:
        with st.expander("🛞 Cambio Gomme Nuovo"):
            ...
            if st.button("Registra Cambio Gomme"):
                ...
                v["data_ultima_inversione"] = oggi_str
                v["storico_interventi"].append({
                    "data": oggi_str, "lavoro": f"SOSTITUZIONE PNEUMATICI NUOVI [{marca_g} - {cod_g}]", "km": km, "costo": costo_g
                })
                Garage.salva(dati)
    ...
    with col_rap3:
        with st.expander("🛑 Pastiglie / Dischi"):
            comp_scelta = st.selectbox("Seleziona Componente", [
            ...
            if st.button("Registra Componente"):
                ...
                oggi_str = datetime.date.today().strftime("%d/%m/%Y")
                v["storico_interventi"].append({
                    "data": oggi_str, "lavoro": f"{nome_c.upper()} [{marca_p} - COD:{cod_p}]", "km": km, "costo": costo_p
                })
                Garage.salva(dati)
    ...
    with st.expander("🛠️ COMPONI TAGLIANDO COMPLETO (OLIO E FILTRI)", expanded=False):
        ...
        for nome_f, chiave_f in filtri_tagliando:
            ...
            prezzo = c_pr.number_input("Prezzo €", value=0.0, key=f"pr_{chiave_f}", disabled=not attivo)
            marca = c_mr.text_input("Marca Ricambio", key=f"mr_{chiave_f}", disabled=not attivo).strip().upper()
            codice = c_cd.text_input("Codice Ricambio", key=f"cd_{chiave_f}", disabled=not attivo).strip()
            
            spec_o = None
            if chiave_f == "ultimo_cambio_olio":
                spec_o = c_sp.text_input("Specifica Olio (es. 5W-30)", key="spec_o", disabled=not attivo).strip().upper()
            ...
            for chiav, (att, prz, mrc, cod, spc) in dati_tagliando_form.items():
                if att:
                    ...
                    if chiav == "ultimo_cambio_olio" and spc:
                        ...
                        nome_pulito = chiav.replace("ultimo_", "").replace("_", " ").upper()
                    
                    filtri_cambiati.append(f"{nome_pulito} ({mrc_t} - COD:{cod_t}) [{prz}€]")
            
            if filtri_cambiati:
                descrizione = "TAGLIANDO COMPLETO: " + ", ".join(filtri_cambiati)
                v["storico_interventi"].append({
                    "data": oggi_str, "lavoro": descrizione, "km": km, "costo": costo_totale
                })
                Garage.salva(dati)
    ...
    # 5. NOTE E PROMEMORIA DIARIO (INCLUSIONE ESPLICITA DELLA DATA NEL TESTO NOTA)
    ...
    col_n2.button("💾 Aggiungi Nota Oggi", key="btn_salva_oggi", on_click=aggiungi_nota_oggi_cb)
    ...
    if not storico_lista:
        ...
        for anno in sorted(interventi_per_anno.keys(), reverse=True):
            ...
            for intv in reversed(interventi_per_anno[anno]):
                st.info(f"🔧 **{intv.get('lavoro')}**  \nData: {intv.get('data')} | Km: {intv.get('km'):,} | Costo: € {intv.get('costo'):.2f}".replace(",", "."))

else:
    st.info("👈 Seleziona un veicolo dal menu a sinistra o creane uno nuovo per iniziare!")
